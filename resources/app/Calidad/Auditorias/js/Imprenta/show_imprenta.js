$(document).ready(function () {
    llenar_tabla_auditoria({ id_reporte_aud_fcc011: parametros[6],tipo: parametros[5]})
    ...
    const mostrar_seccion_impresion_proceso = (param) => {
        $.ajax({
            ...
            data: { codigo: param, tipo: parametros[5] },
    ...
    $(document).on("click", "#tabla_auditoria tbody tr button.ver_detalle_reporte", function () {
        ...
        mostrar_seccion_impresion_proceso(id);
        mostrar_seccion_impresion_producto(id);
        mostrar_valores_agua(id);
        mostrar_seccion_barniz_proceso(id);
        mostrar_seccion_barniz_producto(id);
        mostrar_seccion_tipo_impresion(id);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#btnGetPreviousInfo").attr("data-id", id);
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$.ajax({
					...
					data: { codigo_detalle: $(this).attr("id"), tabla: "detalle_fcc011" },
					success: function (response) { },
				});
